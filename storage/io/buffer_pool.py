...
"""LRU-K(2) 分页缓冲池。
SSTable 按偏移分页缓存，每页 16KB。热页面驻留内存，冷页面淘汰。
淘汰策略：优先淘汰访问次数最少的页（LRU-K 近似）。"""
from collections import OrderedDict
from typing import Any, Dict, List, Optional
import os

PAGE_SIZE = 16 * 1024      # 每页 16KB
DEFAULT_MAX_PAGES = 1024    # 默认最大缓存页数
...
class BufferPool:
    ...
    接口:
      get(page_id) → data | None     — 查缓存
      put(page_id, data)              — 写入缓存
      read_page(path, offset) → data  — 读文件页（自动缓存）
      invalidate(file_or_page)        — 失效缓存
    """
    ...
    def read_page(self, file_path: str,
    ...
    def read_range(self, file_path: str, offset: int,
        ...
        while remaining > 0:
            ...
            if to_read <= 0:
                break
            result.extend(
                page_data[page_offset_within:
                           page_offset_within + to_read])
