...
"""Persistence — binary columnar files with JSON fallback."""
import json
import struct
...
def save(schemas: Dict[str, TableSchema], path: Path) -> None:
    ...
    for name, schema in schemas.items():
        data[name] = {
            'name': schema.name,
            'columns': [{'name': c.name, 'dtype': c.dtype.name,
                          'nullable': c.nullable, 'primary_key': c.primary_key,
                          'max_length': c.max_length} for c in schema.columns],
        }
...
def load(path: Path) -> Dict[str, TableSchema]:
    ...
    for name, td in data.items():
        columns = [ColumnSchema(name=cd['name'], dtype=DataType[cd['dtype']],
...
def save_data(stores: dict, data_dir: Path) -> None:
    """Save table data — try binary first, JSON fallback."""
...
def load_data(stores: dict, data_dir: Path) -> None:
    """Load table data — try binary first, JSON fallback."""
    for name, store in stores.items():
...
def _save_binary(name: str, store: Any, data_dir: Path) -> None:
    from storage.table_file import TableFileWriter, serialize_column
    ...
    writer.finalize(schema_json, n, len(schema.columns), CHUNK_SIZE)


def _load_binary(name: str, store: Any, path: Path) -> None:
    from storage.table_file import TableFileReader, deserialize_column
    ...
    for null_bmps, data_blobs in chunk_groups:
        ...
        for ci in range(min(col_count, len(null_bmps))):
            dtype_name = columns[ci]['dtype'] if ci < len(columns) else 'VARCHAR'
            values, nulls = deserialize_column(null_bmps[ci], data_blobs[ci], n, dtype_name)
            col_values.append(values)

        for ri in range(n):
            row = [col_values[ci][ri] if ci < len(col_values) else None
...
def _save_json(name: str, store: Any, data_dir: Path) -> None:
    ...
    for row in rows:
        ...
        for val in row:
            ...
            elif isinstance(val, (int, float, str)): safe_row.append(val)
