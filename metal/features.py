...
"""特性检测注册表 — 集中检测所有可选组件。
全项目只检测一次，其他模块通过 from metal.features import HAS_XXX 引用。

用法:
    from metal.features import HAS_BLOOM, BloomFilter
    if HAS_BLOOM:
        bf = BloomFilter(1000)

    from metal.features import require_feature
    require_feature('BLOOM')  # 不可用时抛 ImportError
"""
from typing import Optional


def check_feature(name: str) -> bool:
    """检查指定特性是否可用。name 不区分大小写。"""
    return globals().get(f'HAS_{name.upper()}', False)


def require_feature(name: str) -> None:
    """要求指定特性可用，否则抛出 ImportError。"""
    if not check_feature(name):
