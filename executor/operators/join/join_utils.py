...
[D06] NL/Radix/Grace/SMJ 不再各自实现 _eval_cond。"""
...
def batch_eval_join_condition(
    ...
    if n == 0:
        return []
    if n == 1:
        return [eval_join_condition(
            combined_rows[0], schema, on_expr)]
    col_names = [cn for cn, _ in schema]
    col_types = [ct for _, ct in schema]
    rows = [[row.get(cn) for cn in col_names]
            for row in combined_rows]
    batch = VectorBatch.from_rows(rows, col_names, col_types)
    try:
        mask = _SHARED_EVALUATOR.evaluate_predicate(
            on_expr, batch)
