...
"""Pratt 表达式解析器。
通过 ParserInterface 协议与 Parser 交互，不直接访问私有属性。"""
...
_UNRESERVED = frozenset({
    ...
    TokenType.PARTITION, TokenType.ROWS, TokenType.RANGE,
    TokenType.UNBOUNDED, TokenType.PRECEDING,
    TokenType.FOLLOWING, TokenType.CURRENT, TokenType.ROW,
...
class ExprParser:
    ...
    def parse_sort_key(self):
        """解析排序键：expr [ASC|DESC] [NULLS FIRST|LAST]。"""
        expr = self.parse_expression()
        direction = 'ASC'
        if self._p.current_token().type == TokenType.ASC:
            self._p.advance()
        elif self._p.current_token().type == TokenType.DESC:
            direction = 'DESC'
            self._p.advance()
        nulls = None
        if self._p.current_token().type == TokenType.NULLS:
            self._p.advance()
            if self._p.current_token().type == TokenType.FIRST:
                nulls = 'NULLS_FIRST'
                self._p.advance()
            elif self._p.current_token().type == TokenType.LAST:
                nulls = 'NULLS_LAST'
                self._p.advance()
    ...
    def _parse_prefix(self):
        ...
        if tt == TokenType.NOT:
            ...
            return UnaryExpr(
                op='NOT',
                operand=self.parse_expression(Precedence.NOT_PREFIX))
        ...
        if tt == TokenType.EXISTS:
            self._p.advance()
            self._p.expect(TokenType.LPAREN)
            sq = self._p.parse_select()
    ...
    def _parse_identifier_expr(self):
        ...
        if self._p.current_token().type in _UNRESERVED:
            name = name.lower()
        self._p.advance()
        # 无参数函数
        ...
        if self._p.current_token().type == TokenType.DOT:
            ...
            return ColumnRef(
                table=name,
                column=self._p.accept_identifier())
        ...
        if upper in _AGGS:
            if self._p.current_token().type == TokenType.STAR:
                ...
                return self._maybe_window(
                    AggregateCall(name=upper, args=[StarExpr()]))
            distinct = False
            if self._p.current_token().type == TokenType.DISTINCT:
                distinct = True
                self._p.advance()
            args = self._parse_arg_list()
            self._p.expect(TokenType.RPAREN)
            return self._maybe_window(
                AggregateCall(name=upper, args=args,
                              distinct=distinct))
        # 窗口函数
        if upper in _WINFUNCS:
            args = (self._parse_arg_list()
                    if self._p.current_token().type != TokenType.RPAREN
                    else [])
            self._p.expect(TokenType.RPAREN)
            return self._maybe_window(
                FunctionCall(name=upper, args=args))
        # 普通函数
        args = (self._parse_arg_list()
                if self._p.current_token().type != TokenType.RPAREN
                else [])
    ...
    def _maybe_window(self, func):
        """检查是否跟 OVER 窗口规范。"""
        if self._p.current_token().type != TokenType.OVER:
            return func
        self._p.expect(TokenType.OVER)
        self._p.expect(TokenType.LPAREN)
        pb = []
        if self._p.current_token().type == TokenType.PARTITION:
            self._p.advance()
            self._p.expect(TokenType.BY)
            pb = self._parse_arg_list()
        oby = []
        if self._p.current_token().type == TokenType.ORDER:
            self._p.expect(TokenType.ORDER)
            self._p.expect(TokenType.BY)
            ...
            while self._p.current_token().type == TokenType.COMMA:
                ...
                oby.append(self.parse_sort_key())
        frame = None
        if self._p.current_token().type in (
                TokenType.ROWS, TokenType.RANGE):
            frame = self._parse_frame()
        self._p.expect(TokenType.RPAREN)
        return WindowCall(func=func, partition_by=pb,
                          order_by=oby, frame=frame)

    def _parse_frame(self):
        mode = self._p.current_token().value
        self._p.advance()
        ...
        return WindowFrame(
            mode=mode, start=self._parse_bound(),
    ...
    def _parse_case(self):
        ...
        return CaseExpr(operand=op, when_clauses=ws,
                        else_expr=el)

    def _parse_cast(self):
        self._p.expect(TokenType.CAST)
        self._p.expect(TokenType.LPAREN)
        e = self.parse_expression()
        self._p.expect(TokenType.AS)
        t = self._p.parse_type_name()
        self._p.expect(TokenType.RPAREN)
    ...
    def _infix_precedence(self):
        ...
        if tt == TokenType.NOT:
            ...
            if nxt.type in (TokenType.IN, TokenType.BETWEEN,
                            TokenType.LIKE):
    ...
    def _parse_infix(self, left):
        ...
        if tt == TokenType.IN:
            return self._parse_in(left, False)
        if tt == TokenType.BETWEEN:
            return self._parse_between(left, False)
        if tt == TokenType.LIKE:
            return self._parse_like(left, False)
        if tt == TokenType.NOT:
            nxt = self._p.peek_token()
            if nxt.type == TokenType.IN:
                self._p.advance()
                return self._parse_in(left, True)
            if nxt.type == TokenType.BETWEEN:
                self._p.advance()
                return self._parse_between(left, True)
            if nxt.type == TokenType.LIKE:
                self._p.advance()
                return self._parse_like(left, True)
        prec = _INFIX[tt]
        op = self._p.current_token().value
        self._p.advance()
        return BinaryExpr(
    ...
    def _parse_is(self, left):
        self._p.expect(TokenType.IS)
        neg = False
        if self._p.current_token().type == TokenType.NOT:
            neg = True
            self._p.advance()
    ...
    def _parse_in(self, left, negated):
        self._p.expect(TokenType.IN)
        self._p.expect(TokenType.LPAREN)
        if self._p.current_token().type == TokenType.SELECT:
            sq = self._p.parse_select()
            self._p.expect(TokenType.RPAREN)
            return InExpr(expr=left,
                          values=[SubqueryExpr(query=sq)],
                          negated=negated)
