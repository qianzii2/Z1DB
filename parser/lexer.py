...
"""SQL词法分析器。支持双引号标识符、科学计数法、INDEX/USING/NATURAL关键字。"""
...
KEYWORDS: Dict[str, TokenType] = {
    ...
    'JOIN':TokenType.JOIN,'ON':TokenType.ON,'USING':TokenType.USING,'NATURAL':TokenType.NATURAL,
...
class Lexer:
    ...
    def _adv(self):
        ...
        self._p += 1
        self._c = self._s[self._p] if self._p < len(self._s) else None
    ...
    def _num(self) -> Token:
        ln, co = self._ln, self._co; s = self._p; is_float = False
        ...
        if self._c is not None and self._c in ('e', 'E'):
            ...
            if nx is not None and (nx.isdigit() or nx in ('+', '-')):
                ...
                if self._c is None or not self._c.isdigit():
                    raise ParseError("invalid exponent", self._ln, self._co)
                while self._c is not None and self._c.isdigit(): self._adv()
        text = self._s[s:self._p]
        return Token(TokenType.FLOAT_LIT if is_float else TokenType.INTEGER_LIT, text, ln, co)

    def _str(self) -> Token:
        ln, co = self._ln, self._co; self._adv(); p: list = []
        while True:
    ...
    def _quoted_ident(self) -> Token:
        ln, co = self._ln, self._co; self._adv(); p: list = []
        while True:
            if self._c is None: raise ParseError("unterminated quoted identifier", ln, co)
            if self._c == '"':
                if self._pk() == '"': p.append('"'); self._adv(); self._adv()
                else: self._adv(); break
            else: p.append(self._c); self._adv()
